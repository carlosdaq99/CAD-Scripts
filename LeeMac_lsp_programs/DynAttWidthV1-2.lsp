...
;;                                                                      ;;
;;  This program enables the user to dynamically alter the width        ;;
;;  factor of a selected attribute, with a real-time preview as the     ;;
;;  cursor is moved relative to the attribute.                          ;;
;;                                                                      ;;
;;  Upon issuing the command syntax 'attw' at the command-line, the     ;;
;;  user is prompted to select a single-line attribute for which to     ;;
;;  change the width factor.                                            ;;
;;                                                                      ;;
;;  Following a valid selection, the attribute width factor is          ;;
;;  dynamically updated based on the position of the cursor relative    ;;
;;  to the attribute, and the user is presented with various options    ;;
;;  at the command-line.                                                ;;
;;                                                                      ;;
;;  The user may press 'TAB' to toggle between a continuous &           ;;
;;  incremental width factor modification. When the incremental mode    ;;
;;  is enabled, the user may specify the width factor increment by      ;;
;;  pressing 'I' or 'i' and entering any value greater than zero. Each  ;;
;;  of these settings will be remembered between drawing sessions.      ;;
;;                                                                      ;;
;;  The program also allows the user to reset the attribute width       ;;
;;  factor to that of the attribute definition by pressing 'R' or 'r';  ;;
;;  and the program may be cancelled to restore the previous width      ;;
;;  factor by pressing 'C' or 'c' or by pressing 'Esc'.                 ;;
;;                                                                      ;;
;;  The program is restricted to single-line attributes, but will       ;;
;;  perform successfully with attributes at any rotation and inserted   ;;
;;  in any construction plane.                                          ;;
;;----------------------------------------------------------------------;;
...
;;                                                                      ;;
;;  - 'Reset' option now resets the attribute width to match the width  ;;
;;    factor of the attribute definition for the block.                 ;;
;;----------------------------------------------------------------------;;
;;  Version 1.2    -    2014-08-05                                      ;;
;;                                                                      ;;
;;  - Right-clicking acts as 'Cancel' and resets the attribute width.   ;;
;;----------------------------------------------------------------------;;

(defun c:attw ( / *error* bpt dim ent enx fac flg gr1 gr2 inc msg old tag tmp tmx vec wid )

    (defun *error* ( msg )
        (setenv "LMac\\attw-flg" (if flg "1" "0"))
        (if (= 'int (type dim)) (setvar 'dimzin dim))
        (if (and (and msg (wcmatch (strcase msg t) "*break,*cancel*,*exit*"))
                 (numberp old)
            )
    ...
    (setq flg (= "1" (getenv "LMac\\attw-flg"))
          inc (getenv "LMac\\attw-inc")
          dim (getvar 'dimzin)
    )
    (setvar 'dimzin 0)
        ...
        (progn (setvar 'errno 0) (setq ent (car (nentsel "\nSelect attribute: ")))
            (cond
                ...
                (   (member '(101 . "Embedded Object") enx)
                    (princ "\nThis program is not compatible with multiline attributes - sorry.")
                )
            )
        )
    )
            ...
            (princ
                (
                    (setq
                        ...
                        fac
                        (/   ((lambda ( x ) (- (caadr x) (caar x))) (textbox enx))
                ...
                (progn
                    ...
                    (cond
                        (   (or (= 5 gr1) (= 3 gr1))
                            ...
                            (if flg (setq wid (* inc (fix (/ (+ wid (/ inc 2.0)) inc)))))
                         
                            (if (< 0.01 wid 100.0)
                                (progn
                                    (entmod (list ent (cons 41 wid)))
                                    (LM:displaygrtext gr2 (LM:grtext (rtos wid 2 2)) 3 15 -31)
                                )
                                (LM:displaygrtext gr2 (LM:grtext "Invalid Width Factor") 1 15 -31)
                            )
                            (= 5 gr1)
                        )
                        (   (= 2 gr1)
                            (cond
                                (   (= 9 gr2)
                                    (princ (msg (setq flg (not flg))))
                                )
                                (   (and flg (or (= 73 gr2) (= 105 gr2)))
                                    (redraw)
                                 
                                    (initget 6)
                                    (if (setq tmp (getdist (strcat "\nSpecify increment <" (getenv "LMac\\attw-inc") ">: ")))
                                        (setenv "LMac\\attw-inc" (rtos (setq inc tmp) 2 8))
                                    )
                                    (princ (msg flg))
                                )
                                (   (or (= 82 gr2) (= 114 gr2))
                                    (setq tmp (tblobjname "block" (cdr (assoc 2 (entget (cdr (assoc 330 enx))))))
                                          tag (strcase (cdr (assoc 2 enx)))
                                    )
                                    (while (and (setq tmp (entnext tmp)) (null res))
                                        (if
                                            (and
                                                (= "ATTDEF" (cdr (assoc 0 (setq tmx (entget tmp)))))
                                    ...
                                    (prompt (strcat "\nAttribute width reset to " (rtos (cond ((cdr res)) (1.0)) 2 2)))
                                )
                                (   (or (= 67 gr2) (= 99 gr2))
                                    (entmod (list ent (cons 41 old)))
                                    (prompt (strcat "\nProgram cancelled, attribute width reset to " (rtos old 2 2)))
                                )
                            )
                        )
                        (   (= 25 gr1)
...
;;----------------------------=={ GrText }==----------------------------;;
;;                                                                      ;;
;;  Returns a grvecs pixel vector list relative to the origin encoding  ;;
;;  the supplied string.                                                ;;
;;----------------------------------------------------------------------;;
;;  Author: Lee Mac, Copyright © 2013 - www.lee-mac.com                 ;;
;;----------------------------------------------------------------------;;
;;  With thanks to ElpanovEvgeniy for the method of vector encoding to  ;;
...
(princ
    (strcat
        "\n:: DynAttWidth.lsp | Version 1.2 | \\U+00A9 Lee Mac "
